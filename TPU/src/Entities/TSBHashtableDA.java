...
public class TSBHashtableDA<K,V> implements Map<K,V>, Cloneable, Serializable
{
        ...
        private class KeySetIterator implements Iterator<K>
        {
            ...
            public boolean hasNext() 
            {
                ...
                for (int i = next ; i < t.length; i++) {
                    if((t[i]).state == TSBHashtableDA.CLOSED)
                        return true;
                }
                return false;
            }
            ...
            public K next() 
            {
                ...
                int next = current;
                Entry<K, V> t[] = (Entry<K, V>[]) TSBHashtableDA.this.table;
                for (next++ ; t[next].state != TSBHashtableDA.CLOSED; next++);
            ...
            public void remove() 
            {
                ...
                ((Entry<K,V>)TSBHashtableDA.this.table[current]).setState(TOMBSTONE);
                current = last;
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>> 
    {
        ...
        public boolean contains(Object o) 
        {
            ...
            Entry<K, V> entryTable = (Entry<K, V>)TSBHashtableDA.this.search_for_entry(entry.getKey(), ik);
            if(entryTable != null && entryTable.state == TSBHashtableDA.CLOSED) return true;
            return false;
        }
        ...
        public boolean remove(Object o) 
        {
            ...
            int ik = TSBHashtableDA.this.h(entry.getKey());
            int index = TSBHashtableDA.this.search_for_index(entry.getKey(), ik);
            if(index != -1){
                ((Entry<K,V>)TSBHashtableDA.this.table[index]).setState(TSBHashtableDA.TOMBSTONE);
                TSBHashtableDA.this.count--;
                TSBHashtableDA.this.modCount++;
                return true;
            }
        ...
        private class EntrySetIterator implements Iterator<Map.Entry<K, V>>
        {
            ...
            public Map.Entry<K, V> next() 
            {
                ...
                Entry<K, V> t[] = (Entry<K, V>[])TSBHashtableDA.this.table;
                int next = current;
                for (next++ ; ((Entry<K,V>)t[next]).state != TSBHashtableDA.CLOSED; next++);
